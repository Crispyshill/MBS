...
export const getChallenges = async (): Promise<Challenge[]> => {
    try{
    const result = await db.query("SELECT externalId, name, description, points, startdate, enddate, category FROM challenges;");
...
export const getOneChallenge = async(challengeId: string): Promise<Challenge> => {
    try{
    const result = await db.query("SELECT externalId, name, description, points, startdate, enddate, category FROM challenges WHERE externalId = $1;", [challengeId]);
    if(null == result.rowCount || result.rowCount < 1){
...
export const addOneChallenge = async (challenge: Challenge): Promise<boolean> => {
    try {
      ...
      const result = await db.query(
        `INSERT INTO challenges (name, description, points, startdate, category) 
         VALUES ($1, $2, $3, $4, $5)`,
        [challenge.name, challenge.description, challenge.points, startDateUTC, challenge.category]
      );
...
export const updateChallenge = async(challenge: Challenge): Promise<Boolean> => {
    try{
    const result = await db.query("UPDATE challenges SET name = $1, description = $2, points = $3, startdate = $4, enddate = $5, category = $6 WHERE externalid = $7", [challenge.name, challenge.description, challenge.points, challenge.startdate, challenge.enddate, challenge.category, challenge.externalid]);
    return result.rowCount != null && result.rowCount > 0
    }
    catch(error) {
        throw throwDBErrors(error, "Challenge");
